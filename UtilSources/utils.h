...
namespace UTILS {
	...
	class UTILS_exception : public std::exception {
		public:
			...
			UTILS_exception( const std::string m ) {
				message = "UTILS: ";
	...
	T from_string( const std::string& s ) {
		...
		iss >> result;
		//if (iss.fail()) throw UTILS_exception("Unrecognized value in from_string<T>()");
		return result;
	}
	...
	std::string Get_UTILS_Version( );

	void Set_UTILS_Input_Echo( const std::string& name );

	void UTILS_Flush_Echo( );
