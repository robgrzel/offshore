...
namespace UTILS {
	...
	void fft(const std::vector<double> &In, std::vector<std::complex<double>> &Out) {
		...
		std::valarray<std::complex<double>> C(n);
		...
		Out.assign(std::begin(C), std::end(C));
	...
	void fft1d(std::valarray<std::complex<double>> &data, int isign) {
		int n = data.size();
		...
		if (fftwork.find(n) == fftwork.end()) {
			...
			FFTW::CFFTI(n, (double *) (&(work[0])));
		}
		
		vector<double> &work = fftwork[n];
		
		if (isign == 1) {
			FFTW::CFFTB(n, (double *) (&data[0]), (double *) (&(work[0])));
		} else {
			FFTW::CFFTF(n, (double *) (&data[0]), (double *) (&(work[0])));
		}
	}
	...
	void fft1d(CFARRAY data, int N, int isign) {
		...
		if (isign == 1) {
			FFTW::CFFTB(n, (double *) (&data(1)), (double *) (&(work[0])));
		} else {
			FFTW::CFFTF(n, (double *) (&data(1)), (double *) (&(work[0])));
		}
	...
	void fftfreq(std::vector<std::complex<long double>> &f, const double d) {
		...
		std::vector<long long> p1(N);
		std::vector<long long> p2(n / 2);
	...
	size_t fftfreq_pos(std::vector<double> &f, const double d) {
		...
		std::vector<size_t> p1(m);
		
		p1 = arange<size_t>(0, m, 1);
	...
	void fft_pos(std::vector<double> &Out, const std::vector<double> &In, const size_t &Npos) {
		...
		std::vector<std::complex<double>> Out_(n);
		
		fft(In, Out_);
		
		for (size_t i = 0; i < Npos; i++) {
			Out[i] = iamp2psd(Out_[i], Npos);
		}
