...
void Thread_Gui_Control::thread_body_plotter( ) {
	...
	while ( *pdoThread ) {
		if (mglFl) {
			//for ( uint i = 0; i < 3; i++ ) {
				size = size_t( snprintf( nullptr, 0, strKey, redrawCounter ) + 1 ); // Extra space for '\0'
				buf  = new char[size];
				snprintf( buf, size, strKey, redrawCounter );
				mglFl->Clf();
				mglFl->Puts( mglPoint( 0, 0.6 ), buf );
				mglFl->Update();
				//sleep(1);
			//};break; 1 702 586
		};
