...
void ODraw2D::DrawPoly( float x, float y, float r, float angle, ePolyType type ) {
	...
	switch ( type ) {
		case HEXAGON: polygon = hexagon_;
			sz = 6;
			break;
		case SQUARE: polygon = square_;
			sz = 4;
			break;
		case TRIANGLE: polygon = triangle_;
			sz = 3;
			break;
		case CIRCLE: polygon = circle_;
			sz = 20;
			break;
	}
...
void ODraw2D::DrawPointXY(  float x0,  float y0, float z0, float size ) {
	std::cout << "DrawPoinXY\n";
	DrawLine(x0-size,y0,z0,x0+size,y0,z0);
	DrawLine(x0,y0-size,z0,x0,y0+size,z0);

}

void ODraw2D::DrawPointXZ(  float x0,  float y0, float z0, float size ) {
	std::cout << "DrawPointXZ\n";
	DrawLine(x0-size,y0,z0,x0+size,y0,z0,8*size);
	DrawLine(x0,y0,z0-size,x0,y0,z0+size,8*size);

}

void ODraw2D::DrawPointYZ(  float x0,  float y0, float z0, float size ) {
	std::cout << "DrawPointYZ\n";
	DrawLine(x0,y0-size,z0,x0,y0+size,z0);
	DrawLine(x0,y0,z0-size,x0,y0,z0+size);
}


void ODraw2D::DrawPoint(  float x0,  float y0, float z0, float size ) {
	std::cout << "DrawPoint\n";
	DrawLine(x0-size,y0,z0,x0+size,y0,z0);
	DrawLine(x0,y0-size,z0,x0,y0+size,z0);
	DrawLine(x0,y0,z0-size,x0,y0,z0+size);
...
void ODraw2D::DrawLine( float x0, float y0, float z0, float x1, float y1, float z1, float width ) {
	std::cout << "DrawLine\n";
	glColor4f( r_, g_, b_, a_ );
	glLineWidth( width );
...
float ODraw2D::DistanceFromPointToSegment( float x, float y, float x1, float y1, float x2, float y2 ) {
	std::cout << "DistanceFromPointToSegment\n";
	float r;
