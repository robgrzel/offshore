...
template <class T> T t_max( T a, T b, T c ) {
	if (a > b) return t_max( a, c ); else return t_max( b, c );
}
...
template <class T> T t_min( T a, T b, T c ) {
	if (a < b) return t_min( a, c ); else return t_min( b, c );
}
...
static void saveBmp24( int w, int h ) {
	long rP, bPR = ( long ) w * 3L;
	if (bPR % 4L == 0L) rP = 0L; else rP = 4L - (bPR % 4L);
	bPR += rP;
	struct HBmp {
		long  b1L;
		short b2S, b3S;
		long  b4L, b5L, b6L, b7L;
		short b8S, b9S;
		long  b10L, b11L, b12L, b13L, b14L, b15L;
	}    hBmp;
	hBmp.b2S = 0;
	hBmp.b3S = 0;
	hBmp.b4L = 54L;
	...
	hBmp.b14L = 0L;
	hBmp.b15L = 0L;
	hBmp.b1L  = 54L + hBmp.b11L;
	static int imgCnt = 0;
	if (imgCnt == 0) _mkdir( "AVI" );
	++imgCnt;
	std::string fName = "AVI/screen";
	if (imgCnt < 100) if (imgCnt < 10) fName += "00"; else fName += "0";
	char tmpStr[20];
	_itoa_s( imgCnt, tmpStr, 20, 10 );
	fName += tmpStr;
	fName += ".bmp";
	//std::cout << "Saving Screenshot... : " << fileName << std::endl;
	FILE         * f;
	unsigned char* D  = new unsigned char[3 * w * h];
	if (!(fopen_s( &f, fName.c_str(), "wb" ))) {
		delete[] D;
		return;
	}
	fwrite( "BM", 1, 2 * sizeof( char ), f );
	fwrite( &hBmp, 1, sizeof( HBmp ), f );
	glReadPixels( 0, 0, w, h, GL_RGB, GL_UNSIGNED_BYTE, &D[0] );
	int i, j;
	for ( j = 0; j < h; ++j ) {
		for ( i = 0; i < w; ++i ) {
			fwrite( &D[3 * (j * w + i) + 2], 1, 1, f );
			fwrite( &D[3 * (j * w + i) + 1], 1, 1, f );
			fwrite( &D[3 * (j * w + i) + 0], 1, 1, f );
		}
