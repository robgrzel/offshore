function [wk,err,iter] = Wavenumber(g,wa,wd,h,dotu,v,tol)
% Solves the wave dispersion relation sig^2 = g*k*tanh(k*h)
%
% Usage:
%  >> wk = wavenumber(g,wa,wd,h,dotu,v,tol)
%  >> [wk,err] = wavenumber(g,wa,wd,h,dotu,v)
%  >> wavenumber %demo
%
% Description:
...
%   sig = wa - wk*cos(wd)*dotu - wk*cos(wd)*v = wa - wk*uk [rad/T]
%   uk = cos(wd)*dotu + sin(wd)*v [L/T]
%   dotu = current velocity in x direction [L/T]
...
if nargin==0
    ...
    h = 6; %water depth [m]
    dotu = -1;  %current velocity in x direction [m/s]
