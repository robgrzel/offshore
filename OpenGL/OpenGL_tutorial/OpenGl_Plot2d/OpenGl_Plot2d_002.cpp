...
void Gl_Plot_Graph::special( int key, int x, int y ) {
	switch ( key ) {
		case GLUT_KEY_LEFT: offset_x -= 0.03;
			break;
		case GLUT_KEY_RIGHT: offset_x += 0.03;
			break;
		case GLUT_KEY_UP: scale_x *= 1.5;
			break;
		case GLUT_KEY_DOWN: scale_x /= 1.5;
			break;
		case GLUT_KEY_HOME: offset_x = 0.0;
			scale_x                  = 1.0;
			break;
	}
...
Gl_Plot_Graph& Gl_Plot_Graph::get_instance( ){
	static Gl_Plot_Graph gpg;
	return gpg;
}
...
int Gl_Plot_Graph::init_glsl_program( ) {
	program = create_program(
			"../OpenGl_Plot2d/graph002.v.glsl",
			"../OpenGl_Plot2d/graph002.f.glsl"
	                        );
	if (!program) return 1;
	...
	if (attribute_coord2d == -1 || uniform_transform == -1 || uniform_color == -1) return 1;
...
int Gl_Plot_Graph::init_resources( ) {
	...
	glsl_status = init_glsl_program();

	if (glsl_status) return 1;
...
Gl_Plot_Graph::Gl_Plot_Graph( ) {
	...
	init_status = init_resources();
	if (init_status) exit( EXIT_FAILURE );
